label amethyst_1:
    scene bg house night

    "You spent most of the day watching TV and pigging out on the couch."
    "As if summoned by the display of sheer laziness, Amthyst comes out of her room to check up on you."

    show amethyst neutral with dissolve

    a "Hey."

    "She makes herself comfortable next to you."
    "Easier to grab the bowl of chips that way."

    scene bg house night with dissolve

    "As day fades into afternoon, Amethsyt makes her boredom known with an exagerated yawn."


    a "Man I'm bored."
    a "These are all re-runs dude."

    m "It's been hours, Amy."
    m "You're only pointing that out now?"

    a "Hey, I never said I was gonna miss out on some TV time."

    "You shrug nonchalantly."

    m "It's all new to me."
    m "I spent most of my time cramming, I can't really help it."

    "That's mostly true."

    show amethyst happy night

    a "Oh! I got an idea."

    hide amethyst night with dissolve

    "She dissappears into her room with surprising quickness."
    "When she wants to, she can get really motivated about something."
    "You hear some muffled rummaging and just as suddenly, she's back on the couch."

    show amethyst happy night with dissolve

    "In her hand you see a video game case."

    a "While you were gone, 'Battle Bastards 2' came out!"
    a "I practiced it over the winter, just so I could whoop you at it."

    "You feign surprise."
    "When you weren't studying for midterms or finals, you were actually head of the local game club."
    "Pretty much all you did in your free time was play 'Battle Bastards 2'."
    "You were something of a local champ."

    m "Uh sure, I guess."
    m "I'll have to get used to the controls again."

    "Amethsyt can't help but overflow with confidence."

    a "Naw, you've escaped your licks for too long."
    a "We gotta play for something interesting."

    "You think about it, and decide to agree."
    "It isn't often you get one over on Amethyst."


    show amethyst neutral night

    a "Alright uh..."
    a "Loser has to..."

    show amethyst flirty night

    a "Give up the rest of their share of the 'Chaps'!"

    "Your favorite salty snack?"
    "It's a hard offer to pass up."

    m "Okay."
    m "You're on."

    # Gamer music goes Here

    hide amethyst with dissolve

    "It's a fierce competition to be sure."
    "But you've been swapping strategies with some of the best players in the county."
    "And you aren't gonna give up your precious 'Chaps' anytime soon."
    "With sweat soaked thumbs you deliver the final blow."

    "A Winner is You!"

    show amethyst embarrassed night with dissolve

    a "Damn it!"

    "You gloat smugly over your victory."

    a "Now way, [playerName], you cheated!"

    m "Nuh uh, how could I, even?"

    show amethyst neutral night

    a "You were playing this game instead of studying, weren't you?"

    "Poorly, you attempt to hide nervous sweat."

    m "That's not- how's that cheating?"

    a "Psh, you cheated, dude."

    m "You can't hold out on our deal!"

    a "Well I'm not giving up snacks."

    "She looks thoughtful for a moment."

    a "But I bet I can give you something you'd like better."

    "You're suspsicious at first, but relent."
    "It's all in good fun anyhow."

    show amy1 1 with dissolve

    m "...!"

    show amy1 2 with dissolve
    show amy1 3 with dissolve

    m "Wow..."
    a "Yeah I knew you'd like these."
    a "I've seen the way you stare."
    a "You were so obvious dude."

    "Her tits are so large for her size and you can see their obvious heft."

    a "I think you've waited long enough to ger your hands on these."

    "You raise a hand to caress her massive orbs."
    "They feel as firm as they look."
    "Yet still soft enough to be enjoyably malleable."
    "Flicking your thumb across her hard nipple elicits a moan from the shortstack gem."

    a "Okay yeah you're real good at that."
    a "Now let me show you what I can do."

    "Amethyst haphazardly pulls your pants off at the waist."
    "Her tits flop around recklessly as you're just able to get the zipper down."
    "With her prize finally in sight, Amethyst gets to work."

    show amy1 4 with dissolve

    "Your prick is quickly covered in saliva as Amethyst's tongue dutifully covers every inch it can."
    "Her movements are sloppy and quick."
    "You can tell she's just as impatient to get to the 'good' part as you are."
    "After a minute of tongue bathing, Amethyst lets up to admire her handiwork."
    "Hesitation isn't really her thing, but as she brings her pillowy lips close..."

    show amy1 5 with dissolve

    "...the suspense of what's to come nearly overwhelms you."
    "Amethyst's breaths are soft and warm."
    "Before you can say anything and ruin the moment, she lowers her mouth onto your hard cock."

    show amy1 6 with dissolve

    "Her lips slowly push down toward your groin before languidly making their way back to the peak of your dick."
    "In spite of her earlier haste, she now sucks your dick with a intenful laziness."
    "Seeing her smile with a mouth full of dick makes your member twitch."
    "Amethyst moans, pleased at your reaction."
    "Slowly she goes, up and down, never needing to stop and take a breath."
    "Another quirk of being a gem."
    "'Slurp'"
    "'slurp'"
    "..."
    "'slurp'"
    "After what feels like a torturous amount of time, you can't hold yourself back."

    show amy1 7 with dissolve

    m "Ah yeah, come on Amethyst!"

    "You grab the back of her head with both hands."
    "It's time for you to set the pace."
    "She doesn't seem to mind anyway."
    "You quicken into a proper face fucking, her hair bouncing around your fingers."
    "From this angle you can see her impossible booty beneath her."
    "With the speed you've picked up, her ass is also jiggling slightly along with your movements."
    "Neither of you mind Amethyst being used like this."
    "She started it, after all."

    show amy1 8 with dissolve

    "All too soon, the pressure builds in your loins and you're ready to let loose."
    "You're so caught in the moment you don't even bother to warn her."
    "You just start cumming in Amethyst's heavenly mouth."
    "She looks so pleased with herself as her relentless sucking doesn't stop."
    "In fact, she ratchets up the vacuum a notch, her cheeks hollowing out."
    "It seems she's determined to drain you for all your worth."
    "Just as you start to get weak in the knees, you empty the last shot of cum down Amethyst's waiting throat."
    "With your balls drained, you slump backward onto the couch."

    show amy1 9 with dissolve

    "As your eyes refocus, Amethyst shows you the product of your hard work."

    show amy1 10 with dissolve
    a "'Gulp!'"
    show amy1 11 with dissolve

    m "Amy that's gross."
    a "Nah you love it, big guy."

    hide amy1 with dissolve
    "With the night finished and your lust sated, you drift off to sleep right in the living room."


    jump living_room

return
